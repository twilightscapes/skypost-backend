...
class NotesWorkspace {
  ...
  async scheduleCurrentNote() {
    console.log('[Schedule] scheduleCurrentNote called');
    if (!this.currentNote) {
      console.warn('[Schedule] No current note');
      alert('Please create a post first');
      return;
    }

    const datetimeStr = document.getElementById('schedule-datetime').value;
    console.log('[Schedule] Datetime value:', datetimeStr);
    if (!datetimeStr) {
    ...
    try {
      ...
      console.log('[Schedule] Scheduled for:', new Date(scheduledFor));
      ...
      await this.db.saveNote(this.currentNote);
      console.log('[Schedule] Note saved');

      this.hideScheduleModal();
      console.log('[Schedule] Modal hidden');
...
class BlueskyIntegration {
  ...
  async handlePost(e) {
    ...
    try {
      ...
      console.log('[handlePost] Post successful! URI:', responseData.uri);
      ...
      if (responseData.uri) {
        if (this.currentNote) {
          ...
          console.log('[handlePost] ✓ Note with postUri saved');
        }
      } else {
